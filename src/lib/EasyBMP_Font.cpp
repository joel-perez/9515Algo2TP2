...
int PrintString(BMP& Image, char* String, int TopLeftX, int TopLeftY,
	...
	//int CharNumber = 0; //Agregado para evitar warnings...
	unsigned int CharNumber = 0; //Agregado para evitar warnings...
	int StartX = TopLeftX;
	...
	for (CharNumber = 0; CharNumber < strlen(String); CharNumber++) {
		int ReturnPosition = PrintLetter(Image, String[CharNumber], StartX,
				TopLeftY, Height, Color);
...
int PrintLetter(BMP& Image, char Letter, int TopLeftX, int TopLeftY, int Height,
	...
	//RGBApixel TempColor; //Agregado para evitar warnings...
	//TempColor.Red = 0; //Agregado para evitar warnings...
	//TempColor.Green = 255; //Agregado para evitar warnings...
	//TempColor.Blue = 0; //Agregado para evitar warnings...
...
//  DrawLine(Image,x5,y5,x6,y6,Color);
	...
	if (Letter == 's') {
		...
		double MaxAngle1 = 0;
		//double MaxAngle2 = pi; //Agregado para evitar warnings...
		if (difference < 1.5) {
			difference = 1.5;
			MaxAngle1 = 0;
			//MaxAngle2 = 1.5; //Agregado para evitar warnings...
			x1 = TopLeftX + difference;
			x2 = x1;
		}
	...
	if (Letter == 'B') {
		...
		DrawArc(Image, x7, y7, 0.25 * Height, -1.57079632679490,
				1.57079632679490, Color);
		DrawArc(Image, x8, y8, 0.25 * Height, -1.57079632679490,
				1.57079632679490, Color);
	...
	if (Letter == 'P') {
		...
		//int x6 = x3; //Agregado para evitar warnings...
		//int y6 = y1; //Agregado para evitar warnings...
	...
	if (Letter == 'Q') {
		...
		DrawArc(Image, x5, y5, Center, 3.14159265358979, 6.28318530717959,
				Color);
		DrawArc(Image, x6, y6, Center, 0, 3.14159265358979, Color);
	...
	if (Letter == 'S') {
		...
		DrawArc(Image, x1, y1, 0.25 * Height, 1.5707963267948, 6.28318530717,
				Color);
		DrawArc(Image, x2, y2, 0.25 * Height, -1.5707963267948, 3.1415926535897,
				Color);
	...
	if (Letter == '3') {
		...
		DrawArc(Image, x1, y1, 0.25 * Height, -3.14159265358979,
				1.57079632679490, Color);
		DrawArc(Image, x2, y2, 0.25 * Height, -1.57079632679490,
				3.14159265358979, Color);
		DrawLine(Image, x3, y3, x4, y4, Color);
	...
	if (Letter == '-') {
		...
		//int TempRad = (TempWidth - 1) / 2; //Agregado para evitar warnings...
	...
	if (Letter == '_') {
		DrawLine(Image, TopLeftX, TopLeftY + Height, TopLeftX + Width,
				TopLeftY + Height, Color);
	...
	if (Letter == '^') {
		DrawLine(Image, TopLeftX, TopLeftY + Center, TopLeftX + Center,
				TopLeftY, Color);
		DrawLine(Image, TopLeftX + Center, TopLeftY, TopLeftX + Width,
				TopLeftY + Center, Color);
	...
	if (Letter == '&') {
		...
		//int x3 = ebmpRound(TopLeftX + 0.5 * Height); //Agregado para evitar warnings...
		//int y3 = ebmpRound(y2); //Agregado para evitar warnings...

		//int x4 = TopLeftX; //Agregado para evitar warnings...
		//int y4 = ebmpRound(y1); //Agregado para evitar warnings...
