...
BMFH::BMFH() {
	...
	bfSize = 0; //Agregado para evitar warnings...
	bfOffBits = 0; //Agregado para evitar warnings...
}
...
BMIH::BMIH() {
	...
	biBitCount = 0; //Agregado para evitar warnings...
	biSize = 0; //Agregado para evitar warnings...
	biWidth = 0; //Agregado para evitar warnings...
	biHeight = 0; //Agregado para evitar warnings...
	biSizeImage = 0; //Agregado para evitar warnings...
}
...
void BMIH::display(void) {
	using namespace std;
	cout << "biSize: " << (int) biSize << endl << "biWidth: " << (int) biWidth
			<< endl << "biHeight: " << (int) biHeight << endl << "biPlanes: "
			<< (int) biPlanes << endl << "biBitCount: " << (int) biBitCount
			<< endl << "biCompression: " << (int) biCompression << endl
			<< "biSizeImage: " << (int) biSizeImage << endl
			<< "biXPelsPerMeter: " << (int) biXPelsPerMeter << endl
			<< "biYPelsPerMeter: " << (int) biYPelsPerMeter << endl
			<< "biClrUsed: " << (int) biClrUsed << endl << "biClrImportant: "
			<< (int) biClrImportant << endl << endl;
}

void BMFH::display(void) {
	using namespace std;
	cout << "bfType: " << (int) bfType << endl << "bfSize: " << (int) bfSize
			<< endl << "bfReserved1: " << (int) bfReserved1 << endl
			<< "bfReserved2: " << (int) bfReserved2 << endl << "bfOffBits: "
			<< (int) bfOffBits << endl << endl;
}
...
bool BMP::SetColor(int ColorNumber, RGBApixel NewColor) {
	...
	if (!Colors) {
		if (EasyBMPwarnings) {
			cout
					<< "EasyBMP Warning: Attempted to set a color, but the color table"
					<< endl
...
RGBApixel BMP::GetColor(int ColorNumber) {
	...
	if (!Colors) {
		if (EasyBMPwarnings) {
			cout << "EasyBMP Warning: Requested a color, but the color table"
					<< endl
					<< "                 is not defined. Ignoring request."
					<< endl;
		}
...
bool BMP::SetBitDepth(int NewDepth) {
	...
	if (NewDepth != 1 && NewDepth != 4 && NewDepth != 8 && NewDepth != 16
		...
		if (EasyBMPwarnings) {
			cout
					<< "EasyBMP Warning: User attempted to set unsupported bit depth "
					<< NewDepth << "." << endl
...
bool BMP::SetSize(int NewWidth, int NewHeight) {
	...
	if (NewWidth <= 0 || NewHeight <= 0) {
		if (EasyBMPwarnings) {
			cout
					<< "EasyBMP Warning: User attempted to set a non-positive width or height."
					<< endl << "                 Size remains unchanged at "
					<< Width << " x " << Height << "." << endl;
		}
		return false;
	}
...
bool BMP::WriteToFile(const char* FileName) {
	...
	if (fp == NULL) {
		if (EasyBMPwarnings) {
			cout << "EasyBMP Error: Cannot open file " << FileName
					<< " for output." << endl;
		}
		fclose(fp);
		return false;
	...
	if (BitDepth != 16) {
		...
		while (j > -1) {
			...
			if (Success) {
				int BytesWritten = (int) fwrite((char*) Buffer, 1, BufferSize,
						fp);
			...
			if (!Success) {
				if (EasyBMPwarnings) {
					cout
							<< "EasyBMP Error: Could not write proper amount of data."
							<< endl;
				}
				j = -1;
			}
...
bool BMP::ReadFromFile(const char* FileName) {
	...
	if (bmih.biCompression > 3) {
		if (EasyBMPwarnings) {
			cout << "EasyBMP Error: " << FileName
					<< " is in an unsupported format." << endl
					<< "               (bmih.biCompression = "
	...
	if (TempBitDepth != 1 && TempBitDepth != 4 && TempBitDepth != 8
		...
		if (EasyBMPwarnings) {
			cout << "EasyBMP Error: " << FileName
					<< " has unrecognized bit depth." << endl;
		}
		SetSize(1, 1);
		SetBitDepth(1);
		fclose(fp);
	...
	if (BitDepth < 16) {
		...
		if (NumberOfColorsToRead < TellNumberOfColors()) {
			if (EasyBMPwarnings) {
				cout << "EasyBMP Warning: file " << FileName
						<< " has an underspecified" << endl
	...
	if (BitDepth != 16) {
		...
		while (j > -1) {
			...
			if (BytesRead < BufferSize) {
				...
				if (EasyBMPwarnings) {
					cout
							<< "EasyBMP Error: Could not read proper amount of data."
							<< endl;
				}
			} else {
				bool Success = false;
				...
				if (!Success) {
					if (EasyBMPwarnings) {
						cout
								<< "EasyBMP Error: Could not read enough pixel data!"
								<< endl;
					}
					j = -1;
				}
...
void DisplayBitmapInfo(const char* szFileNameIn) {
	...
	cout << "BITMAPFILEHEADER:" << endl << "bfType: " << bmfh.bfType << endl
			<< "bfSize: " << bmfh.bfSize << endl << "bfReserved1: "
			<< bmfh.bfReserved1 << endl << "bfReserved2: " << bmfh.bfReserved2
			<< endl << "bfOffBits: " << bmfh.bfOffBits << endl << endl;

	cout << "BITMAPINFOHEADER:" << endl << "biSize: " << bmih.biSize << endl
			<< "biWidth: " << bmih.biWidth << endl << "biHeight: "
			<< bmih.biHeight << endl << "biPlanes: " << bmih.biPlanes << endl
			<< "biBitCount: " << bmih.biBitCount << endl << "biCompression: "
			<< bmih.biCompression << endl << "biSizeImage: " << bmih.biSizeImage
			<< endl << "biXPelsPerMeter: " << bmih.biXPelsPerMeter << endl
			<< "biYPelsPerMeter: " << bmih.biYPelsPerMeter << endl
			<< "biClrUsed: " << bmih.biClrUsed << endl << "biClrImportant: "
			<< bmih.biClrImportant << endl << endl;
	return;
}
...
void RangedPixelToPixelCopy(BMP& From, int FromL, int FromR, int FromB,
	...
	for (j = FromT; j <= FromB; j++) {
		for (i = FromL; i <= FromR; i++) {
			PixelToPixelCopy(From, i, j, To, ToX + (i - FromL),
					ToY + (j - FromT));
		}
	}
...
void RangedPixelToPixelCopyTransparent(BMP& From, int FromL, int FromR,
		int FromB, int FromT, BMP& To, int ToX, int ToY,
...
bool BMP::Write4bitRow(ebmpBYTE* Buffer, int BufferSize, int Row) {
	...
	while (i < Width) {
		...
		int Index = 0;
		while (j < 2 && i < Width) {
			Index += (PositionWeights[j]
					* (int) FindClosestColor(Pixels[i][Row]));
			i++;
			j++;
		}
...
bool Rescale(BMP& InputImage, char mode, int NewDimension) {
	...
	for (int j = 0; j < NewHeight - 1; j++) {
		...
		for (int i = 0; i < NewWidth - 1; i++) {
			...
			InputImage(i, j)->Red = (ebmpBYTE) ((1.0 - ThetaI - ThetaJ
					+ ThetaI * ThetaJ) * (OldImage(I, J)->Red)
			...
			InputImage(i, j)->Green = (ebmpBYTE) ((1.0 - ThetaI - ThetaJ
					+ ThetaI * ThetaJ) * OldImage(I, J)->Green
			...
			InputImage(i, j)->Blue = (ebmpBYTE) ((1.0 - ThetaI - ThetaJ
					+ ThetaI * ThetaJ) * OldImage(I, J)->Blue
