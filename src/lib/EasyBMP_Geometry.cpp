...
double LineFunction(double SlopeX, double SlopeY, int StartX, int StartY,
		double TestX, double TestY) {
	return fabs(SlopeX * (TestY - StartY) - SlopeY * (TestX - StartX));
}

void DrawAALine(BMP &Image, int FromX, int FromY, int ToX, int ToY,
		RGBApixel Color) {
		...
		for (j = LeftJ; j <= RightJ; j++) {
			...
			for (ii = -2; ii <= 1; ii++) {
				for (jj = -2; jj <= 1; jj++) {
					x = i + ii * dx + 0.5 * dx;
					y = j + jj * dy + 0.5 * dy;
					double Temp1 = LineFunction(SlopeX, SlopeY, FromX, FromY, x,
							y);
					if (Temp1 <= 0.5) {
						Temp1 = 1.0;
					} else {
			...
			if (Temp > MinValue) {
				Image(i, j)->Red = (ebmpBYTE) (unsigned int) ((1.0 - Temp)
						* ((double) Image(i, j)->Red)
						+ Temp * ((double) Color.Red));
				Image(i, j)->Green = (ebmpBYTE) (unsigned int) ((1.0 - Temp)
						* ((double) Image(i, j)->Green)
						+ Temp * ((double) Color.Green));
				Image(i, j)->Blue = (ebmpBYTE) (unsigned int) ((1.0 - Temp)
						* ((double) Image(i, j)->Blue)
...
void DrawFastLine(BMP &Image, int FromX, int FromY, int ToX, int ToY,
		RGBApixel Color) {
	// bounds checking
...
void DrawArc(BMP &Image, double CenterX, double CenterY, double Radius,
	...
	//double dTheta = 1.0 / TempRadius; //Agregado para evitar warnings...
	...
	for (i = 0; i < Downsampled.TellWidth(); i++) {
		for (j = 0; j < Downsampled.TellHeight(); j++) {
			...
			if (DestinationI >= 0 && DestinationI < Image.TellWidth()
				...
				Image(DestinationI, DestinationJ)->Red = (ebmpBYTE) ((1.0
						- alpha) * Image(DestinationI, DestinationJ)->Red
						+ (alpha) * Color.Red);
				Image(DestinationI, DestinationJ)->Green = (ebmpBYTE) ((1.0
						- alpha) * Image(DestinationI, DestinationJ)->Green
						+ (alpha) * Color.Green);
				Image(DestinationI, DestinationJ)->Blue = (ebmpBYTE) ((1.0
						- alpha) * Image(DestinationI, DestinationJ)->Blue
						+ (alpha) * Color.Blue);
			}
